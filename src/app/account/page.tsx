...
function AccountInner() {
  ...
  return (
    <AppShell>
      <div className="mx-auto flex min-h-[72vh] max-w-sm flex-col justify-center gap-4 px-2 py-6">
      {/* Level — optional. Default is "Tự động": the feed starts easy and learns
          your level from what you mark OK/Mới. Change it anytime. */}
      <div className="post fade-up p-5">
        <div className="mb-3 flex items-center gap-2 font-semibold">
          <GraduationCap size={18} className="text-[var(--accent)]" /> Trình độ
        </div>
        <div className="flex flex-col gap-2">
          {LEVELS.map((o) => {
            // In auto mode the "Tự động" row is the active one, regardless of the
            // level currently inferred.
      ...
      {me ? (
        <div className="post fade-up p-6 text-center">
          <div className="mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--chip)]">
            <CircleUserRound size={30} className="text-[var(--accent)]" />
          </div>
          <div className="text-lg font-semibold">{me.name || me.email}</div>
          <div className="text-sm text-[var(--muted)]">{me.email}</div>
          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            <button onClick={logout} className="inline-flex items-center justify-center gap-2 rounded-lg border border-[var(--border)] px-4 py-2.5 text-sm">
        ...
        <div className="post fade-up p-6">
          ...
          {googleEnabled && (
            <>
              <a href="/api/auth/google/start" className="mb-4 flex items-center justify-center gap-2 rounded-lg border border-[var(--border)] bg-white px-4 py-2.5 font-semibold">
